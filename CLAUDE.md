...
- `app/src/main/java/com/checkin/app/`
  ...
  - `ui/checkin/`, `ui/attendance/`, `ui/reports/`, `ui/settings/`, `ui/camera/` (+ `AuthGate`), `ui/navigation/` (`AppNavScaffold`), `ui/components/` (`EmptyState`, `ConstrainedContent`, `LocalSnackbarHostState`) + `ui/components/charts/` (`ChartGeometry` + `CircularProgressRing`/`DonutChart`/`LineChart`/`BarChart`), `ui/theme/` (`statusColor` in `Color.kt`)
...
- **Two SharedPreferences namespaces.** `attendance_prefs` (settings + `target_schedule` + `tracking_start_date`, keys **and readers** — `readSchedule`/`readTrackingStart` — centralized in `data/AttendancePrefs`, used by all four ViewModels and `CheckInService`); `checkin_timer_prefs` (live timer + reminder + pause state in `CheckInService`).
- **The nav scaffold owns the screen title.** `AppNavScaffold` renders a `CenterAlignedTopAppBar` whose title is the active `Screen.titleRes`, driven by the same back-stack subscription as the bottom nav — screens never draw their own heading, and they receive the combined top inset through the Scaffold's `innerPadding`. `SettingsViewModel` is the one VM that is prefs-only (no Room `Flow`), so it re-reads on resume and after each write rather than building a reactive pipeline.
...
- **The Attendance calendar is sized from the viewport.** `CalendarGrid` takes a `cellHeight` that `AttendanceScreen` derives from `screenHeightDp` and the month's real row count (4–6, so the grid doesn't jump between months), clamped to `48.dp..80.dp` — a cell is only ~53dp wide, so an uncapped height stretches it into a ribbon. Selecting a day **replaces** the month-summary card rather than appending below it, so picking a date never adds height. Both averages are per *tracked* day, which keeps absent days in the denominator; the all-time one reuses the same tracking-start→yesterday range query that previously computed the deficit, so it costs nothing extra.
...
- **The Check-In screen is a fixed, non-scrolling layout — keep it that way.** The gauge and the primary button must both be reachable in one viewport and one thumb reach, so the screen is a weighted `Column`, not a `LazyColumn`, and the day's sessions collapse behind a tap. A weighted `Column` *clips* rather than scrolls when content outgrows the viewport, so `BoxWithConstraints` picks a compact scrolling branch below `560.dp` of height (landscape, very small phones, large font scales) and sizes the gauge from the available height. Adding anything to this screen means re-checking that it still fits — and note the compact branch cannot host a `LazyColumn` (it would be measured with unbounded height and crash), which is why the expanded session list has two renderings.
