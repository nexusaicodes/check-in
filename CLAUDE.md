...
- `app/src/main/java/com/checkin/app/`
  ...
  - `ui/checkin/`, `ui/attendance/`, `ui/reports/`, `ui/settings/`, `ui/camera/` (+ `AuthGate`), `ui/navigation/` (`AppNavScaffold`), `ui/components/` (`EmptyState`, `ConstrainedContent`, `LocalSnackbarHostState`) + `ui/components/charts/` (`CircularProgressRing`), `ui/theme/` (`statusColor` in `Color.kt`)
